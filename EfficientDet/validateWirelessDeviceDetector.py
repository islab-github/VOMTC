...
from VOTRE_dataset_forVALTEST import load_val_data, load_val_partial_data
### IF VOTRE:
...
ClipB = ClipBoxes()
PredJson = "predictionWirelessDeviceDetector_VAL_FULL.json"
...
def test(model2):
    ...
    for count, (data_key, (gt_path, rgb_path, distance_path, directory, original_file)) in enumerate(data.items()):
        ...
        img_th = torch.from_numpy(framed_img[0]).to(device1).unsqueeze(0).permute(0, 3, 1, 2)
        prediction = detect(
...
if __name__ == '__main__':
    ...
    data = load_val_data()
    predictionF = load_prediction(json_path="/home/glen/EfficientDet/predictionWirelessDeviceDetector_VAL_FULL.json")
    ...
    elevation_angle_err += (total_samples - tp)*abs(int(np.max(angle)) - 1) #35

...
import copy
from VOMTC_dataset_forVALTEST import load_VOMTCv2_data
...
ClipB = ClipBoxes()
PredJson = "predictionWirelessDeviceDetector_VOMTCv2SET_FULL.json"
...
if __name__ == '__main__':
    ...
    data = load_VOMTCv2_data()
    predictionF = load_prediction(json_path="/home/glen/EfficientDet/predictionWirelessDeviceDetector_VOMTCv2SET_FULL.json")
    ...
    az_angle_err_variance = 0
    el_angle_err_variance = 0 
    
    mean_azimuth_angle_err_initial = 0
    mean_elevation_angle_err_initial =  0
    
    #az_sample_num = 0
    #el_sample_num = 0
    all_rgb_paths  = []
    imagewhererecalldegrades = []
    UE_gt = []
    UE_predicted = []
    UE_distance = []
    UE_indices = []
    ...
    for count, ((gt_path, rgb_path, distance_path, _,_), pred) in enumerate(zip(data.values(), predictionF.values())):
        ...
        with open(gt_path) as f:
            gt_path = f.readlines()
        box_gtt = np.array(gt_path)
        box_gt = []
        for i in range(len(box_gtt)):
            
            if box_gtt[i][0] == '5' or box_gtt[i][0] == '6' or box_gtt[i][0] == '7' or box_gtt[i][0] == '8' or box_gtt[i][0] == '9':
               coordinates = box_gtt[i][1:].split(' ')
               transform_coordinates = boundingbox(float(coordinates[1]), float(coordinates[2]), float(coordinates[3]), float(coordinates[4]) )
               transform_coordinates = np.array(transform_coordinates)
               box_gt.append(transform_coordinates)
        ...
        if prev_rgb_path is not None and prev_rgb_path == rgb_path:
            num_gt = 0  # 이전과 현재의 rgb_path가 동일한 경우에만 num_gt를 0으로 설정
            num_pred = 0
            num_match = 0
        else:
            num_gt = len(box_gt)
        ...
        for ig, ip in zip(idx_gt, idx_pred):
            ...
            elevation_gt = (math.atan(y_gt/x_gt))
            elevation_predicted = (math.atan(y_predicted/x_predicted))
            
            if (r_gt > 0) and (r_predicted > 0 ) and (azimuth_gt > 0) and (azimuth_predicted > 0) and (elevation_gt > 0) and (elevation_predicted > 0):
           
                
                
                x_gt_c = r_gt * math.sin((azimuth_gt*math.pi)/180)* math.cos(elevation_gt)
                x_predicted_c = r_predicted * math.sin((azimuth_predicted*math.pi)/180) * math.cos(elevation_predicted)
            
                y_gt_c = r_gt * math.sin((azimuth_gt*math.pi)/180) * math.sin(elevation_gt)
                y_predicted_c = r_predicted * math.sin((azimuth_gt*math.pi)/180) * math.sin(elevation_predicted)
            
                z_gt = r_gt * math.cos((azimuth_gt*math.pi)/180)
                z_predicted = r_predicted * math.cos((azimuth_gt*math.pi)/180) 
                
                UE_gt.append([x_gt_c, y_gt_c, z_gt])
                UE_predicted.append([x_predicted_c, y_predicted_c, z_predicted])
                print('Coordinates %s %s %s : ' % (x_gt_c, y_gt_c, z_gt))
                print('Predicted Coordinates %s %s %s: ' % (x_predicted_c, y_predicted_c, z_predicted))
    ...
    mean_azimuth_angle_err_initial = azimuth_angle_err / total_samples 
    mean_elevation_angle_err_initial =  elevation_angle_err / total_samples 
    ...
    mean_az_angle_err_variance = az_angle_err_variance / total_samples #max(angle_valid_count, 1)
    mean_el_angle_err_variance = el_angle_err_variance / total_samples #max(angle_valid_count, 1)
    #추가함:
    ...
    print(f"Azimuth Angle ori error: {mean_azimuth_angle_err_initial:6f} deg (valid: {angle_valid_count})")
    print(f"Elevation Angle ori error: {mean_elevation_angle_err_initial:6f} deg (valid: {angle_valid_count})")
    print(f"Variance Az Angle error: {mean_az_angle_err_variance:6f} degree (valid: {angle_valid_count})")
    print(f"Variance El Angle error: {mean_el_angle_err_variance:6f} degree (valid: {angle_valid_count})")
